...
ti = 27;
Cp = 0.480*1000; % joules/kgdegc
A = 53.4751*10^-6; %m^2
m = 8.20*1000*A*L; %kg/m^3
K = 13.0; %w/m-K 
...
tdiv = 0.1;
hc = 2.5; % W/m^2k convective heat transferfor air
...
for i = 2:length(t)
    ...
    %L = 5*10*-3; 
    temp_est_1(i) = (current(i)*voltage(i) - (hc*As +K1*Ac/L1)*(temp_est_1(i-1) - ti) - K*Ac*(temp_est_1(i-1) - temp_est_2(i-1))/L1)*tdiv/( 8.20*1000*A*L*Cp) + temp_est_1(i-1);

    %section 2 T502
    %L = 20*10*-3; 
    temp_est_2(i) = (current(i)*voltage(i) - (hc*As)*(temp_est_2(i-1) - ti) - K*Ac*((temp_est_2(i-1) - temp_est_1(i-1))/L1 + (temp_est_2(i-1) - temp_est_3(i-1))/L2))*tdiv/( 8.20*1000*A*L*Cp) + temp_est_2(i-1);

    %section 3 mounting clip
    %L = 20*10*-3; 
    temp_est_3(i) = (current(i)*voltage(i) - (hc*As +K1*Ac/L3)*(temp_est_3(i-1) - ti) - K*Ac*((temp_est_3(i-1) - temp_est_2(i-1))/L2 + (temp_est_3(i-1) - temp_est_4(i-1))/L3))*tdiv/( 8.20*1000*A*L*Cp) + temp_est_3(i-1);


    %section 4 T501
    %L = 20*10*-3; 
    temp_est_4(i) = (current(i)*voltage(i) - (hc*As)*(temp_est_4(i-1) - ti) - K*Ac*((temp_est_4(i-1) - temp_est_3(i-1))/L3 + (temp_est_4(i-1) - temp_est_5(i-1))/L4))*tdiv/( 8.20*1000*A*L*Cp) + temp_est_4(i-1);


    %section 5 mounting clip
    %L = 20*10*-3; 
    temp_est_5(i) = (current(i)*voltage(i) - (hc*As +K1*Ac/L5)*(temp_est_5(i-1) - ti) - K*Ac*((temp_est_5(i-1) - temp_est_4(i-1))/L4+(temp_est_5(i-1) - temp_est_6(i-1))/L5))*tdiv/( 8.20*1000*A*L*Cp) + temp_est_5(i-1);

    %power 2 terminal and T504
    %L = 20*10*-3; 
    temp_est_6(i) = (current(i)*voltage(i) - (hc*As +K1*Ac/L6)*(temp_est_6(i-1) - ti) - K*Ac*(temp_est_6(i-1) - temp_est_5(i-1))/L5)*tdiv/( 8.20*1000*A*L*Cp) + temp_est_6(i-1);
    ...
    temp_est_basic(i) = (current(i)*voltage(i) - hc*Ac*(temp_est_conv(i-1) - ti) - As*e*emissivity*((temp_est_conv(i-1)+273) - ti))*tdiv/(m*Cp) + temp_est_conv(i-1);
end
...
Ar = A;
A = [1, -hc*Ar*tdiv/(m*Cp)+1];
B = hc*Ar*tdiv/(m*Cp)*ti;
C = [1, 0];
T = [ti];
for i = 1:(length(t)-1)

    Tnew = A*[current(i)*voltage(i); T(i)] + B;
    T = [T,Tnew];
    %T(i) = T(i) + B;
end
